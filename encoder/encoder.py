...
class Encoder:
    ...
    def __count_pulse_callback(self, chip, gpio, level, tick):  
        self.__pulse_count += 1  # increment the pulse count for that motor
    
    def __calculate_error(self, targetPWM, currentRPM):
        if targetPWM > 0:
    ...
    def pi_controller(self, targetPWM, motorSpeed):
        currentRPM = ((self.pulse_count / 1080) * (60 / self.__DT))
       
        error = self.__calculate_error(targetPWM, currentRPM)
